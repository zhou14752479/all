   ...
   public class shuiguo
    {
        ...
        static Dictionary<string, object> GetCaptchaConfig(string captchaUrl, HttpClient client)
        {
            ...
            {
                function.log($"水果获取验证码配置信息时出错：{ex.Message}");
                throw;
            }
        }
        ...
        static void GetX5sec(string url, HttpClient client)
        {
            try
            {
                ...
                var postRequest = new HttpRequestMessage(HttpMethod.Post, "http://220.168.146.21:41019/sgurl");
                ...
                if (x5secResponse.Headers.TryGetValues("Set-Cookie", out cookies))
                {
                    ...
                    {
                        function.log($"水果失败: {x5secResponseContent}");
                        fail++;
                    }
                }
                else
                {
                    function.log($"失败水果: 未获取到 Set-Cookie 头");
                    fail++;
                }

                function.log($"水果当前成功率:{Math.Round((double)success / (success + fail) * 100, 2)} 次数:{(success + fail)}");
            }
            catch (Exception ex)
            {
                function.log($"处理水果 X5Sec 时出错：{ex.Message}");
            }
        }
