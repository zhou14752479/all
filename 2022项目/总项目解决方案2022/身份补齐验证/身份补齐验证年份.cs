...
namespace 身份补齐验证
{
    public partial class 身份补齐验证年份 : Form
    {
        public 身份补齐验证年份()
        {
            InitializeComponent();
        ...
        private void download(string uid)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int year = Convert.ToInt32(textBox4.Text.Trim()); year <= Convert.ToInt32(textBox5.Text.Trim()); year++)
                    {
                        ...
                        for (int month = 1; month <= monthlen; month++)
                        {
                           


                            for (int day = 1; day <= daylen; day++)
                            {
                                ...
                                string card = value[1].Trim().Replace("********", "abcdefgh");
                                card = card.Replace("******", "abcdef");
                                card = card.Replace("****", "abcd");
                                card = card.Replace("**", "gh");

                               
                                card = card.Replace("abcd", year.ToString());
                                card = card.Replace("ef", month2.ToString());
                                card = card.Replace("gh", day2.ToString());
        ...
        #endregion
        private void 身份补齐验证年份_Load(object sender, EventArgs e)
        {
            #region 通用检测
        ...
        private void 身份补齐验证年份_FormClosing(object sender, FormClosingEventArgs e)
        {
