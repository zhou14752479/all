            ...
            try
            {
                string city = "zoucheng";
                    ...
                    for (int i = 1; i < 71; i++)
                    {
                        ...
                        MatchCollection niandai = Regex.Matches(strhtml, @"<span class=""sub"">([\s\S]*?)</span>");
                        ...
                        lv1.SubItems.Add(niandai[2].Groups[1].Value.Trim());
                        lv1.SubItems.Add(Regex.Replace(quyu.Groups[1].Value, "<[^>]*>", "").Trim());
                        lv1.SubItems.Add(Regex.Replace(diduan.Groups[1].Value, "<[^>]*>", "").Trim());
                        lv1.SubItems.Add(Regex.Replace(quyu.Groups[1].Value, "<[^>]*>", "").Trim()+ Regex.Replace(diduan.Groups[1].Value, "<[^>]*>", "").Trim());
